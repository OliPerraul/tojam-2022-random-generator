...
namespace Cirrus.Collections
{
	...
	/// <para>The <see cref="PopFront()"/> methods will remove the items that are returned from the CircularBuffer{T}. To view the contents of the CircularBuffer{T} without removing items, use the <see cref="Peek()"/> or <see cref="PeekLast"/> methods.</para>
	/// </remarks>
	public class CircularBuffer<T> : ICollection<T>, ICollection
	{
		...
		public int PopFront(T[] array)
		{
			return this.PopFront(array, 0, array.Length);
		}
		...
		/// <param name="count">The number of elements to copy.</param>
		/// <returns>The actual number of elements copied into <paramref name="array"/>.</returns>
		public virtual int PopFront(T[] array, int arrayIndex, int count)
		{
			int realCount;
			int dstIndex;
		...
		public virtual bool PopFront(out T item)
		{
			item = default(T);
		...
		public virtual bool PopBack(out T item)
		{
			item = default(T);
