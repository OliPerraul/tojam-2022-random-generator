...
namespace Cirrus.Collections.Queues
{
	/// <summary>
	...
	public class BasicPriorityQueue<TItem, TPriority> : IPriorityQueue<TItem, TPriority>
		where TPriority : IComparable<TPriority>
	{
		...
		private readonly GenericPriorityQueue<BasicNode, TPriority> _queue;
		private readonly Dictionary<TItem, IList<BasicNode>> _itemToNodesCache;
		private readonly IList<BasicNode> _nullNodesCache;

		#region Constructors
		/// <summary>
		/// Instantiate a new Priority Queue
		/// </summary>
		public BasicPriorityQueue() : this(Comparer<TPriority>.Default, EqualityComparer<TItem>.Default) { }

		/// <summary>
		/// Instantiate a new Priority Queue
		/// </summary>
		/// <param name="priorityComparer">The comparer used to compare TPriority values.  Defaults to Comparer&lt;TPriority&gt;.default</param>
		public BasicPriorityQueue(IComparer<TPriority> priorityComparer) : this(priorityComparer.Compare, EqualityComparer<TItem>.Default) { }

		/// <summary>
		/// Instantiate a new Priority Queue
		/// </summary>
		/// <param name="priorityComparer">The comparison function to use to compare TPriority values</param>
		public BasicPriorityQueue(Comparison<TPriority> priorityComparer) : this(priorityComparer, EqualityComparer<TItem>.Default) { }

		/// <summary>
		/// Instantiate a new Priority Queue       
		/// </summary>
		/// <param name="itemEquality">The equality comparison function to use to compare TItem values</param>
		public BasicPriorityQueue(IEqualityComparer<TItem> itemEquality) : this(Comparer<TPriority>.Default, itemEquality) { }
		...
		/// <param name="priorityComparer">The comparer used to compare TPriority values.  Defaults to Comparer&lt;TPriority&gt;.default</param>
		/// <param name="itemEquality">The equality comparison function to use to compare TItem values</param>
		public BasicPriorityQueue(IComparer<TPriority> priorityComparer, IEqualityComparer<TItem> itemEquality) : this(priorityComparer.Compare, itemEquality) { }
		...
		/// <param name="priorityComparer">The comparison function to use to compare TPriority values</param>
		/// <param name="itemEquality">The equality comparison function to use to compare TItem values</param>
		public BasicPriorityQueue(Comparison<TPriority> priorityComparer, IEqualityComparer<TItem> itemEquality)
		{
			_queue = new GenericPriorityQueue<BasicNode, TPriority>(INITIAL_QUEUE_SIZE, priorityComparer);
			_itemToNodesCache = new Dictionary<TItem, IList<BasicNode>>(itemEquality);
			_nullNodesCache = new System.Collections.Generic.List<BasicNode>();
		}
		#endregion

		/// <summary>
		/// Given an item of type T, returns the existing BasicNode in the queue
		/// </summary>
		private BasicNode GetExistingNode(TItem item)
		{
			if(item == null)
			{
		...
		/// </summary>
		private void AddToNodeCache(BasicNode node)
		{
			if(node.Data == null)
			{
			...
			{
				nodes = new System.Collections.Generic.List<BasicNode>();
		...
		/// </summary>
		private void RemoveFromNodeCache(BasicNode node)
		{
			if(node.Data == null)
			{
		...
		/// <returns></returns>
		private BasicNode EnqueueNoLockOrCache(TItem item, TPriority priority)
		{
			BasicNode node = new BasicNode(item);
		...
		public void Enqueue(TItem item, TPriority priority)
		{
			lock(_queue)
			{
				...
				BasicNode node = EnqueueNoLockOrCache(item, priority);
				nodes.Add(node);
			}
		}
		...
		public void UpdatePriority(TItem item, TPriority priority)
		{
			lock(_queue)
			{
				BasicNode updateMe = GetExistingNode(item);
				if(updateMe == null)
				{
		...
		public TPriority GetPriority(TItem item)
		{
			lock(_queue)
			{
				BasicNode findMe = GetExistingNode(item);
				if(findMe == null)
				{
		...
		{
			System.Collections.Generic.List<TItem> queueData = new System.Collections.Generic.List<TItem>();
			lock(_queue)
			{
		...
		public bool IsValidQueue()
		{
			lock(_queue)
			{
				...
				foreach(IList<BasicNode> nodes in _itemToNodesCache.Values)
				{
				...
				foreach(BasicNode node in _queue)
				{
	...
	/// A simplified priority queue implementation.  Is stable, auto-resizes, and thread-safe, at the cost of being slightly slower than
	/// FastPriorityQueue
	/// This class is kept here for backwards compatibility.  It's recommended you use BasicPriorityQueue&lt;TItem, TPriority&gt;
	/// </summary>
	/// <typeparam name="TItem">The type to enqueue</typeparam>
	public class BasicPriorityQueue<TItem> : BasicPriorityQueue<TItem, float>
	{
		/// <summary>
		/// Instantiate a new Priority Queue
		/// </summary>
		...
		public BasicPriorityQueue(IComparer<float> comparer) : base(comparer) { }

		/// <summary>
		/// Instantiate a new Priority Queue
		/// </summary>
		/// <param name="comparer">The comparison function to use to compare priority values</param>
		public BasicPriorityQueue(Comparison<float> comparer) : base(comparer) { }
	}
}