...
namespace Cirrus.Unity
{
	public static class PhysicsUtils
	{
		public static void IgnoreLayersCollision(int layers1, int layers2)
		{
			for(int i = 0; i < 32; i++)
			{
				if(((1 << i) & layers1) != 0)
		...
		public static int BoxCastNonAlloc(
			BoxCollider box, Vector3 direction,
			RaycastHit[] results, float maxDistance = Mathf.Infinity,
			int layerMask = Physics.DefaultRaycastLayers,
			QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal)
		{
			Vector3 center, halfExtents;
