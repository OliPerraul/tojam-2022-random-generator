...
namespace Cirrus.Collections
{
	...
	public class BHeap<T> : IEnumerable<T>
	{
		...
		public BHeap(SortDirection sortDirection, IEnumerable<T> initial, IComparer<T> comparer)
		{
			...
			if(initial != null)
			{
				var items = initial as T[] ?? Enumerable.ToArray(initial);
				var initArray = new T[Enumerable.Count(items)];
	...
	public class SimpleBHeap1<T> : BHeap<T>
		where T : IPriorityEntry
	{
		public SimpleBHeap1(SortDirection sortDirection)
			: base(
				sortDirection,
				null,
	...
	public class SimpleBHeap2<T> : BHeap<Tuple<T, float>>
	{
		public SimpleBHeap2(SortDirection sortDirection)
			: base(
				sortDirection,
				null,
				Comparer<Tuple<T, float>>.Create((x, y) =>
				{
		...
		public void Insert(T newItem, float prio)
		{
			base.Insert(new Tuple<T, float>(newItem, prio));
		}
	}
}